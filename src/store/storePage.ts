...
import { AS, DC, XSD } from '../namespaces';
...
export async function storePage(store: $rdf.IndexedFormula, webId: string, page: Page): Promise<$rdf.Node> {
  ...
  if (typeof storedPage === 'undefined') {
    ...
    store.add(pageIndex, DC('references'), newPage, pageIndex),
      store.add(newPage, AS('type'), AS('Page'), newPage.doc()),
      store.add(newPage, DC('created'), $rdf.lit(page.created.toISOString(), null as any, XSD('dateTime')), newPage.doc()),
